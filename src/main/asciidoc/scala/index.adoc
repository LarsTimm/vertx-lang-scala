...
You can't do much in Vert.x-land unless you can commune with a `link:scaladocs/io/vertx/scala/core/Vertx.html[Vertx]` object!
...
It's done by calling `link:scaladocs/io/vertx/scala/core/Vertx.html#executeBlocking(io.vertx.core.Handler,%20io.vertx.core.Handler)[executeBlocking]` specifying both the blocking code to execute and a
...
This configuration is then available via the `link:scaladocs/io/vertx/scala/core/Context.html[Context]` object.
